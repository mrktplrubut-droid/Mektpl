...
async def pay_file(call: CallbackQuery):
    ...
    if not data:
        ...
        await call.message.answer(
            (
                "⚠️ <b>QR OTOMATIS1 TIDAK TERSEDIA</b>\n\n"
                "Sistem QR Otomatis1 sedang mengalami gangguan.\n\n"
...
# ==================================================
# AUTO PAYMENT QRIS 2 - DOMPETX
# ==================================================

@router.callback_query(F.data.startswith("dompetx:"))
async def pay_file_dompetx(call: CallbackQuery):

    user_id = call.from_user.id
    code = call.data.split(":", 1)[1]

    await call.answer(
        "⏳ Membuat QR Otomatis2..."
    )
    ...
    if not data:
        ...
        await call.message.answer(
            (
                "⚠️ <b>QR OTOMATIS2 TIDAK TERSEDIA</b>\n\n"
                "Sistem QR Otomatis2 sedang mengalami gangguan.\n\n"
                "Silahkan gunakan QR Manual."
            ),
            parse_mode="HTML",
            reply_markup=fallback_auto2_keyboard(code)
        )
    ...
    if not payment_id or not qr_string:
        ...
        logger.error(
            "DOMPETX INVALID PAYMENT DATA: %s",
            data
        )
        ...
        await call.message.answer(
            (
                "⚠️ <b>QR OTOMATIS2 TIDAK TERSEDIA</b>\n\n"
                "QR dari provider tidak dapat dibuat.\n\n"
                "Silahkan gunakan QR Manual."
            ),
            parse_mode="HTML",
            reply_markup=fallback_auto2_keyboard(code)
    ...
    try:
        ...
        if qr_image:
            ...
            # Kalau DompetX memberikan URL gambar
            if isinstance(qr_image, str) and qr_image.startswith(
                ("http://", "https://")
            ):

                async with httpx.AsyncClient(timeout=30) as client:

                    response = await client.get(qr_image)
        ...
        logger.exception(
            "DOMPETX QR IMAGE ERROR"
        )

        # Kalau gambar resmi gagal, tetap generate
        # QR dari qr_string
